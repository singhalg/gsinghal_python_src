...
##    files = [ ['PT_10_lung_1200384.vcf','']]  #/////----///---////----
    ...
    allDeleterious = {}  # here we are considering only deleterious mutations and only in metastasis tumors.
...
##    del commonEVS, agilentCommon,IlluminaCommon,Illumina_agilent,dbSNPcommon
    ...
    for eachTumor in files:
        ...
        delMutationsMetastasis = pickle.load(fh)
        fh.close()

        end = delMutPickleName.find('_brain')   ####----------------==============----------===========-------------=============
        patientName = delMutPickleName[:end]

        delMutLocations = delMutationsMetastasis.keys()
        for eachLoc in delMutLocations:  #/////----///---////----
            AF =  delMutationsMetastasis[eachLoc]['AF']
            DP = delMutationsMetastasis[eachLoc]['DP']
            ALT =  delMutationsMetastasis[eachLoc]['ALT']
        ...
        ##                            newVal = [[patientName, ALT[i], float(AF[i]), DP[0]]]
                ...
                if (eachLoc in allDeleterious) and (eachLoc not in mutation_info_dict):       # adding only those variants to mut_info which have been added to allDeleterious
                    info = delMutationsMetastasis[eachLoc]
                    mut_info = []
...
##                    predictions = {}
        ...
        del delMutationsMetastasis, delMutLocations


    allDeleteriousPickle = open('All_deleterious_metastasis_rare.pkl', 'w')  #/////----///---////----
    pickle.dump(allDeleterious, allDeleteriousPickle)
    allDeleteriousPickle.close()

    mut_info_pickle = open('metastasis_mutation_info_dict_new.pkl', 'w')  #/////----///---////----
    pickle.dump(mutation_info_dict, mut_info_pickle)
    mut_info_pickle.close()

    del mutation_info_dict, allDeleterious
    print 'Finished reading metastatic mutations'
...
##    allCommon = commonEVS | agilentCommon | IlluminaCommon | Illumina_agilent | dbSNPcommon
    ...
    fhAllDeleterious = open('All_deleterious_metastasis_rare.pkl', 'rU') #/////----///---////----
...
##        if eachLoc not in allCommon: # here, we are taking only those locations which are not present in any of the internal or external common snps
    ...
    fhRecurrentMutList = open('met_recurrent_mutation_list.pkl', 'w')  #/////----///---////----
...
##        print eachMostRecurrent, '   # of patients having this mutation = ', eachMostRecurrent[1]
    ...
    print 'Finished calculating recurrence in metastasis'
...
def recurrentMut2CSV():
    recurrentMutList = pickle.load(open('met_recurrent_mutation_list.pkl', 'rU')) #/////----///---////----
    allDeleterious = pickle.load(open('All_deleterious_metastasis_rare.pkl', 'rU')) #/////----///---////----
    mutation_info_dict = pickle.load(open('metastasis_mutation_info_dict_new.pkl', 'rU'))   #/////----///---////----
    ...
    fhout = open('mutation_recurrence_results_metastasis.csv', 'w')
