...
def fastqDownload(sampleName):
    ...
    fn = 'gsinghal'+sampleName+'.download_log'
    log = open(fn, 'w')
...
##            oldcmd = " wget --retry-connrefused -b -q "+ file_loc
    ...
    timestamp =  ' It took '+ str(datetime.timedelta(seconds=end)) + ' to complete the downloading. \n'
    print timestamp
    log.write(timestamp)

    st2 = time.time()
    print 'Starting chksum ... '

    for each in file_PID_dict.keys():
...
##            gunzipProcs.append(job)
    ...
    if len (tbDwnld - Dwnld) > 0:
        log.write('These files were not downloaded \n')
        badFiles = ','.join(list(tbDwnld-Dwnld))
        log.write(badFiles)
        log.write('\n')
        ExitSwitch = True
...
##        aproc.wait()
    ...
    timestamp =  ' It took '+ str(datetime.timedelta(seconds=end)) + ' to complete the unpacking. \n'
    print timestamp
    log.write(timestamp)
...
def bashScript(nodes, ppn, wt, working_dir, reference, samplename):
    ...
    jobname1 = 'gsinghal_' + samplename + '_'+reference+ '_paired'
    jobname2 = 'gsinghal_' + samplename + '_'+reference+ '_se'
    bsName1 = jobname1
    bsName2 = jobname2
...
# writing bowtie command for paired mates
    ...
    command = 'bowtie2 -p ' + proc +'  --sensitive  -X 800  -x  ' + reference+ '  -1 ' + mate1[:-1] + ' -2 ' + mate2[:-1] +  ' -S '+outFile1 +' &>  '+ stdoutFile
...
##    runBS(bsName1)
##    runBS(bsName2)
##    bsName1, bsName2 = bashScript('1', '487', '12:00:00' , '/scratch/gsinghal/YRI_LC/' , 'hg19', sampleName)
##    runBS(bsName1)
##    runBS(bsName2)
