...
def fetchSeq(left_flank, right_flank):
    ...
    variantData = open('variants_for_validation_seq_v0.csv', 'rU').readlines()

    outfileName ='variants_for_validation_' + left_flank + '_' + right_flank + '.csv'
    ...
    for each in variantData[1:]:
        ...
        flds = each.split(',')
        key = 'chr'+ flds[2]
        start = int(flds[3]) - int(left_flank)
        end = int(flds[3])+ int(right_flank)
