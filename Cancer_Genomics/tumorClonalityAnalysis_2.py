...
def pileupCompare(file1, file2, outfileName, targetCapturePlatform, databases):
    ...
    if targetCapturePlatform not in targetCaptureFlds:
        ...
        sys.exit()
    AgV3, AgV4, Illumina, EVS, dbSNPcommon, v3_exp, v4_exp, v4_v3_exp = databases
    fh1 = open(file1, 'rU')
    fh2 = open(file2, 'rU')
...
##    AgV3, AgV4, Illumina, EVS, dbSNPcommon
    ...
    if targetCapturePlatform == 'AgilentV3':
        allControls = AgV3 | EVS | dbSNPcommon |v3_exp | v4_v3_exp

    elif targetCapturePlatform == 'AgilentV4':
        allControls = AgV4 | EVS | dbSNPcommon | v4_exp | v4_v3_exp

    else:
        allControls = Illumina | EVS | dbSNPcommon  # not filtering for anything else for Illumina vcf
...
##    print 'reached point 3'
                                                                            # While making V4, I have used the v3_exp.pkl and v4_exp.pkl. These were the variants which were found to be common in all vcfs belonging to either V3 or V4 exp platforms.
    outfile1 = outfileName + '_no_commonV4.csv'          #===============----------------=================--------   Here, I have used commonEVS_asrd10_maf1.5.pkl as the pickle for common variants in EVS data.
    outfile2 = outfileName + '_no_commonV4_cutoff.csv'   #===============----------------=================--------  While making V2, I used commonEVS_asrd10.pkl
...
##    fhexternal = open('commonEVS_asrd10.pkl', 'rU')
    fhexternal = open('commonEVS_asrd10_maf1.5.pkl', 'rU')
    ...
    fhV3Exp = open('V3_exp_common.pkl', 'rU')
    v3_exp = pickle.load(fhV3Exp)
    fhV3Exp.close()


    fhV4Exp = open('V4_exp_common.pkl', 'rU')
    v4_exp = pickle.load(fhV4Exp)
    fhV4Exp.close()


    fhV4V3Exp = open('V4_V3_exp_common.pkl', 'rU')
    v4_v3_exp = pickle.load(fhV4V3Exp)
    fhV4V3Exp.close()



    databases = AgV3, AgV4, Illumina, EVS, dbSNPcommon, v3_exp, v4_exp, v4_v3_exp
