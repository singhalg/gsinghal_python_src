...
def fastqDownload(sampleName):
    ...
    for line in seq_data:
        ...
        if (flds[9].strip() == sampleName) and (flds[20].strip()== '0')  and (flds[12].strip()=='ILLUMINA'):
            ...
            cmd = " wget --retry-connrefused -b -q "+ file_loc

##            # in case the ascp does not work, replace the cmd with oldcmd .
##            cmd = 'ascp -i ~/.aspera/connect/etc/asperaweb_id_dsa.putty -Q -l 100M  fasp-g1k@fasp.1000genomes.ebi.ac.uk:vol1/ftp/'+flds[0].strip()
...
##    fastqDownload(sampleName)
    ...
    bsName1, bsName2 = bashScript('1', '40', '12:00:00' , '/scratch/gsinghal/YRI_LC/' , 'YRIref_index', sampleName)
    runBS(bsName1)
    runBS(bsName2)
    bsName1, bsName2 = bashScript('1', '40', '12:00:00' , '/scratch/gsinghal/YRI_LC/' , 'hg19', sampleName)
    runBS(bsName1)
    runBS(bsName2)
