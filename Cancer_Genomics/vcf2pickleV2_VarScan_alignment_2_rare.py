...
##    fhout = open('variant_detection_stats.csv', 'w')
...
##    fhout.write(outline)
    ...
    for eachPatient in files:
        for eachFile in eachPatient:
            ...
            tissue_identifier = pt_name + tissue
...
##        fileName = eachFile[:end] + '_brain_VarScan_clean_snpEff_SnpSift.vcf'  #-----========---------========--------
            ...
            variantVCF, rareVCF = readVCF(fileName, allCommon)
...
##            fhAllVCF.close()
            ...
            rareVCFpickleName = fileName[:-4]+'_rare.pkl'
            fhDelVCF = open(rareVCFpickleName, 'w')
            pickle.dump(rareVCF, fhDelVCF)
            fhDelVCF.close()

##            deleteriousRareVCFPickle = fileName[:-4]+'_rare_del.pkl'
...
##        if eachPos not in commonSnpSet:
