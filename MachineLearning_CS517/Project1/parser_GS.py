...
def crossValidateImputation():
    patients = nearestNeighbor()
    
    numPatients = len(patients)
    numGenes = len(patients[0])
    for i in range (10000):
        i = rd.randint(0,numPatients)
        j = rd.randint(0, numGenes)
...
def crossValidateImputation():
    ...
    patients = nearestNeighbor('train_2.csv')
    ...
    genes = []
    for y in range(numGenes):
        genes.append(patientsArr[:,y])
    
    meanGeneVals= []
    for aGene in genes:
        meanGeneVals.append(mean2(aGene))
     
    for i in range(numPatients):
        for j in range(numGenes):
            if (patientsArr[i][j]==float('NaN')) or (patientsArr[i][j]==float('NaN\n')) :
                patientsArr[i][j]=meanGeneVals[j]
    
    # we will use the patientsArr to fit the nearest neighbor model
    ...
    for i in range(numPatients):
        for j in range(numGenes):
            if (patientsC[i][j]=='NaN') or (patientsC[i][j]=='NaN\n') :
                nbrs = neigh.kneighbors(patientsArr[i])
                knbrs = nbrs[1]
                patientsC[i][j]= retMissingVal(knbrs, patientsArr, j)
            else:
                patientsC[i][j] = float(patientsC[i][j])
    ...
    for aLoc in locations:
        # compareVals is a list of pairs, each pair being [ starting data value, imputed value replacing NaN] 
        compareVals.append([ patients[aLoc[0]][aLoc[1]] , patientsC[aLoc[0]][aLoc[1]]  ])
    
    print compareVals[:20]   
