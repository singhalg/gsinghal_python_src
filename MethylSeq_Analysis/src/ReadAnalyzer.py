...
plz send queries to gsinghal@wustl.edu

'''

####################################################################################################
# README PLEASE
# This is the key script which calculates per base scores for a repeat element consensus
# using a set of reads and repeatmasker output. Fundamentally, it calculates the intersection 
# between :
# 1. Repeat Masker output for a particular repeat element and  
# 2. reads from any sequencing read out.
#
# Syntax : 
# python ReadAnalyzer.py <readfile> <repeatFile1>
#
# The output will be written to a file named as "repeatElement_read_score" 
# The respective <repeatElement> is picked from the input file
# The output file contains scores for each base of the Repeat Element consensus. 
# The scores represent how often does each base of the Repeat Element consensus 
# appear in the reads. The output file also contains count of total # of reads in the <readfile> 
# # of reads that intersect with the repeat element
#
# To achieve functionality, this script requires the following scripts in the same directory
# 1. ReadParser.py 
# 2. Parser1.py
# 3. Overlap.py
#
#  This script processes one <readfile> for one <repeatFile>. However, it can be scaled up 
#  to process 21 <repeatFile> sequentially by commenting some code in SuperReadAnalyzer() and 
#  main() methods. 
#
#  Approx run time on sandbox3 (3.0Ghz, 6Gb Memory) is 30 mins, with 5Gb of memory consumption.
#  Memory consumption is approx twice of the size of <readfile>; so for a <readfile> of size 2.5 Gb, 
#  this script needs 5.0 Gb of memory (yulk!!!)
#  
#
###################################################################################################
...
def SuperReadAnalyzer(ReadData, Repeat1
#   UNCOMMENT this to process 20 more repeat elements together                    
...
'''
This script prints the output to a file
'''
...
#    Repeat21 = sys.argv[22]
    ...
    SuperReadAnalyzer(ReadData, Repeat1
# Uncomment this to process 20 more repeat elements together                      
#                      , Repeat2, Repeat3, Repeat4,
#                      Repeat5, Repeat6, Repeat7, Repeat8, Repeat9, 
#                      Repeat10, Repeat11, Repeat12, Repeat13, Repeat14,
