...
##    files = [ 'PT_10_brain_1200360.vcf','']
    ...
    for eachPatient in files:
        eachFile = eachPatient[0]
        fileName = eachFile[:-4]+'_snpEff_SnpSift.vcf'
...
##    print deleteriousVariantsVCF
    ...
    topPos = variantsVCF.keys()
    for each in topPos[100:500]:
        print each
...
def main():
##    VCFtoPickle()
    files = [   ['PT_10_lung_1200384_snpEff_SnpSift.vcf','PT_10_brain_1200360.vcf']]
    fileName = files[0][0]
    commonSnpSet = Set([])
    readVCF(fileName, commonSnpSet)
