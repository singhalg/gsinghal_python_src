...
def blah():
	test_data = convert_file2dictionary("test.tsv","TargetID")
	fTasks = open("test.tsv","r")
	original_lines = fTask.readlines()
	fTasks.close()
	names = []

	names = test_data.keys()
	orig_names = joblib.load("features.pkl")

	missing = ""
	for each in orig_names:
		if each not in names:
			missing = each
	print "missing: %s, index is: %s" %(missing, orig_names.index(missing))
...
##	fobj.close()
	...
	#drawStuff()
	result = predict("NB","Univariate","2")
	print result
	new_result = []
	for each in result:
		if each > 0:
			new_result.append("Normal")
		else:
			new_result.append("AD")
	print new_result
